  ...
  if (props.user === null) {
    return (
      <IconButton id='favorite-button'  onClick={() => alert('please sign up or login to use this feature')} size='large' >
        <StarOutlineIcon style={{width: '20px', height: '20px'}} nClick={() => alert('please sign up or login to use this feature')}/>
      </IconButton>
      )
    }
  ...
  return (
   ...
   return (
     <Tooltip title="Remove Favorite">
       <IconButton id='unfavorite-button' size='large'  onClick={() => props.handleButtonPress(props.recipe.id)}>
        <StarIcon id='unfavorite-button' style={{width: '20px', height: '20px'}} onClick={() => props.handleButtonPress(props.recipe.id)}/>
      </IconButton>
     </Tooltip>
   )
 }
